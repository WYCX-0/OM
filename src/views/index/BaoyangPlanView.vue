  ...
  <div class="customer-service-container">
    ...
    <div class="table-container">
      <el-table :data="tableData" border v-loading="loading" style="width: 100%">
        ...
        <el-table-column prop="deviceName" label="保养设备" width="150"></el-table-column>
        <el-table-column prop="frequency" label="保养频次" width="150">
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="50%" :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="保养频次" prop="frequency">
          <el-select v-model="form.frequency" placeholder="请选择保养频次">
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      this.loading = true;
      const baoyangPlanPageDTO = {
        page: this.pagination.currentPage,
        pageSize: this.pagination.pageSize,
        status: this.searchForm.status,
        deviceId: this.searchForm.deviceId
      };
      request.post('/admin/baoyangPlan/list', baoyangPlanPageDTO, {
        headers: {
          'Authorization': `Bearer ${this.token}`
        }
      }).then(response => {
    ...
    handleStatusChange(row) {
      ...
      this.loading = true;
      request.post(`/admin/baoyangPlan/${newStatus}?id=${row.id}`, {}, {
        headers: {
          'Authorization': `Bearer ${this.token}`
        }
      }).then(res => {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          const url = this.dialogType === "add"
              ? '/admin/baoyangPlan/add'
              : '/admin/baoyangPlan/update';
