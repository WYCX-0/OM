  ...
  onLoad(options) {
    if (options.detailData) {
      try {
        ...
        this.getFenceInfo();
        common_vendor.index.__f__("log", "at pages/baoyangOrder/baoyangOrder.vue:130", this.detailData);
      } catch (e) {
        common_vendor.index.__f__("error", "at pages/baoyangOrder/baoyangOrder.vue:132", "解析详情数据失败", e);
  ...
  methods: {
    async startProcessing() {
      try {
        ...
        if (confirmResult.confirm) {
          ...
          const requestResult = await new Promise((resolve, reject) => {
            common_vendor.index.request({
              url: `${utils_config.baseConfig.baseUrl}/engineer/baoyangOrder/deal/${this.detailData.id}`,
              method: "POST",
              header: {
        ...
        common_vendor.index.__f__("error", "at pages/baoyangOrder/baoyangOrder.vue:228", "处理过程中出现错误:", error);
        common_vendor.index.showToast({
          title: "处理失败",
          icon: "none"
        });
    ...
    async updateUserPosition() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/baoyangOrder/baoyangOrder.vue:327", "获取位置失败:", error);
      }
    },
    // 定位检查方法
    async checkLocation() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/baoyangOrder/baoyangOrder.vue:344", "检查位置时出错:", error);
      }
    },
    // 检查是否在围栏内
    ...
    startLocationCheck() {
      ...
      this.locationInterval = setInterval(async () => {
        try {
          ...
          common_vendor.index.__f__("error", "at pages/baoyangOrder/baoyangOrder.vue:452", "周期定位检查失败:", e);
        }
      }, 5e3);
    },
    // 获取当前位置
          ...
          common_vendor.index.uploadFile({
            ...
            success: (uploadRes) => {
              try {
                ...
                common_vendor.index.__f__("error", "at pages/baoyangOrder/baoyangOrder.vue:502", "解析响应失败", e);
                common_vendor.index.showToast({
                  title: "上传结果解析失败",
                  icon: "none"
                });
            ...
            fail: (err) => {
              common_vendor.index.__f__("error", "at pages/baoyangOrder/baoyangOrder.vue:510", "上传失败", err);
              common_vendor.index.showToast({
    ...
    completeRepair() {
      ...
      common_vendor.index.__f__("log", "at pages/baoyangOrder/baoyangOrder.vue:535", "finish:{}", this.finishUrl);
      common_vendor.index.showModal({
        title: "确认完成",
        content: "确定已完成维修吗？",
        success: (res) => {
          common_vendor.index.__f__("log", "at pages/baoyangOrder/baoyangOrder.vue:540", this.current);
          if (res.confirm) {
            common_vendor.index.request({
              url: `${utils_config.baseConfig.baseUrl}/engineer/baoyangOrder/finish/${this.detailData.id}`,
              method: "POST",
              header: {
                "Authorization": `Bearer ${this.token}`,
    ...
    formatBasicDate(dateArray) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/baoyangOrder/baoyangOrder.vue:594", "日期格式化失败:", e);
        return "--";
      }
    },
    getAccessToken() {
    ...
    getDeviceOptions() {
      common_vendor.index.request({
        ...
        fail: (err) => {
          common_vendor.index.__f__("error", "at pages/baoyangOrder/baoyangOrder.vue:623", "获取设备列表失败", err);
          common_vendor.index.showToast({
    ...
    updateStatus(newStatus) {
      ...
      common_vendor.index.__f__("log", "at pages/baoyangOrder/baoyangOrder.vue:669", "状态已更新为:", this.getStatusText(newStatus));
    },
    onRadioChange(e) {
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-6915bd98"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages/baoyangOrder/baoyangOrder.js.map
